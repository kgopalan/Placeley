...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self){
        ...
        self.layer.position = CGPointMake(frame.size.width/2 + frame.origin.x, frame.size.height/2 + frame.origin.y);
    }
    return self;
}
    ...
    if ([animation.keyPath isEqualToString:@"transform.scale"] && flag){
        switch (self.state) {
            case FAFancyButtonFadeIn:
                ...
                CGAffineTransform transform = CGAffineTransformMakeRotation(DegreesToRadians(self.degree));
                transform = CGAffineTransformTranslate(transform, 0, -49);
